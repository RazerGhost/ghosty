...
import { Atom, Braces, Code2, Database, FileCode2 } from "lucide-react";
...
export function TechStackTile({
  ...
  const stop = () =>
    autoplay && autoplayRef.current && autoplayRef.current.stop();
  const play = () =>
    autoplay && autoplayRef.current && autoplayRef.current.play();
          ...
          <div className="flex">
            {items.map((it, i) => (
              <div
                ...
                role="group"
              >
                <TechSlide accent={it.accent} icon={it.icon} label={it.label} />
              </div>
            ))}
          </div>
        </div>
...
function TechSlide({
  ...
  return (
    <Tooltip content={label}>
      <button
        className=" appearance-none group relative overflow-hidden h-12 md:h-14 rounded-xl border border-foreground/10 p-3 md:p-3.5 transition hover:border-foreground/20 hover:shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 bg-transparent"
        type="button"
      >
        {/* gradient layer */}
        <span
