...
export function DiscordActivityTile({
  ...
  if (loading || !status)
    return <TileSkeleton body={body} card={card} header={header} size={size} />;
  ...
  return (
    <Card className={`${size} ${card} relative overflow-hidden`}>
      <div
        className={`pointer-events-none absolute inset-0 bg-gradient-to-br ${wash} to-transparent`}
      />

      <CardHeader
        className={`${header} flex items-center justify-between relative z-10`}
      >
      ...
      <CardBody className={`${body} relative z-10 flex flex-col gap-3`}>
        ...
        {playing.length > 0 ? (
          <div className="flex flex-col gap-2">
            {playing.map((act) => (
              <PlayingRow
                key={act.id ?? `${act.name}-${act.created_at}`}
                act={act}
              />
            ))}
          </div>
