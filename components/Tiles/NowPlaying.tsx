...
export function NowPlayingTile({
  ...
  useEffect(() => {
    ...
    const fetchLastPlayed = async () => {
      try {
        ...
        // defer to avoid setState directly in mount effect
        queueMicrotask(() => setSpotifyData(data.song));
      } catch (err) {
  ...
  const listening = status
    ? activitiesOfType(status.activities, 2)[0]
    : undefined;
  ...
  useEffect(() => {
    ...
    if (spotify) {
      ...
      if (incomingId !== currentId) {
        ...
        queueMicrotask(() => setSpotifyData(spotify));
      }
    }
  ...
  useEffect(() => {
    ...
    if (wasLive && !isLive) {
      ...
      if (last) {
        ...
        queueMicrotask(() => setSpotifyData(pausedSnapshot));
      }
    }

    // Defer setState to avoid calling setState synchronously inside the effect
    queueMicrotask(() => setWasLive(isLive));
  }, [spotify, elapsed, duration, wasLive]);

  const [isPlaying, setIsPlaying] = useState(false);

  // Determine whether the track is currently playing. Avoid calling Date.now() during render
  // to satisfy react-hooks/purity. Update once per second while spotify is live.
  useEffect(() => {
    if (!start || !end) {
      // Defer to avoid calling setState synchronously inside effect
    ...
    const check = () => setIsPlaying(!!(spotify && Date.now() < end));

    // Run check immediately and then every second while spotify is live
    check();
    if (spotify) {
  ...
  useEffect(() => {
    if (!start || !end || end <= start) {
      // Defer multiple state updates to avoid cascading renders from an effect
      queueMicrotask(() => {
        setProgress(0);
        setElapsed(0);
        setDuration(0);
    ...
    if (spotify) {
      // perform an initial update but defer the reactive setState to avoid cascading renders
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="basis-1/4 min-w-0">
            <div className="relative aspect-square rounded-xl overflow-hidden bg-foreground/10">
              ...
              {albumArt ? (
                <Image
                  alt={album ?? song ?? "Unknown track"}
                  className={`w-full h-full object-cover ${
                    !isPlaying ? "opacity-80 grayscale" : ""
                  }`}
                  src={albumArt}
                />
              ) : (
